...
select * from departamento; -- FROM indica a tabela que usaremos para a consulta

desc empregado;
select codigo, nome, cpf, depto_num from empregado;
select distinct nome,depto_num from empregado; -- DISTINCT TIRA VALOR DUPLICADO
...
select *
	from dependente where parentesco = 'Filho'
    and sexo = 'M' -- AND une mais umma CONDIÇÃO
    and codigo_emp = 13; 
    
select * from dependente 
    where codigo_emp = 12 or -- OR ou condicional
    codigo_emp = 13 or
    codigo_emp = 18;
...
select codigo, nome, depto_num, sexo from empregado
	where (depto_num = 20 or depto_num = 50) and sexo = 'F'; -- AND é mais forte que o OR
...
select cpf, nome, upper(nome) as nome_maiusculo from empregado
		where lower(nome) = 'neymar';

select nome, length(nome) tam_nome from empregado;

select logradouro, substr(logradouro, 3) from empregado; -- SUBSTR pega parte de uma string

select logradouro, nro, concat(logradouro, ', ' , nro) as Endreco from empregado; -- CONCAT contaneção

select salario, round(salario,1) from empregado; -- ROUND arredondamento por numero de casas decimais

select salario, round(salario,1), truncate(salario,1) from empregado; -- TRUNCATE corta o digito

select mod(salario, 100) from empregado; -- MOD resto da divisão
...
select nome, coalesce(replace(replace (sexo, 'M', 'Masc.'), 'F', 'Fem.'), 'N/I') from empregado;
