...
select e.codigo Emp_codigo, e.nome Emp_nome, d.numero Depto_num, d.nome Depto_nome
	from empregado e join departamento d on e.depto_num = d.numero;
...
select p.numero projeto_num, p.nome projeto_nome, d.nome departamento_nome
	from projeto p join departamento d on p.depto_num = d.numero;
...
select e.codigo emp_codigo, e.nome emp_nome, e.sexo emp_sexo, t.horas horas_trabalhadas
	from empregado e join trabalha_em t on e.codigo = t.codigo_emp;
...
select e2.nome nome_gerente, e2.codigo, e1.nome nome_subordinado
	from empregado e1 join empregado e2 on e1.codigo_ger = e2.codigo
...
select e.nome nome, e.salario salario, p.nome
	from trabalha_em t join empregado e on t.codigo_emp = e.codigo
...
select e.nome nome, e.salario salario, p.nome
	from trabalha_em t join projeto p on t.projeto_num = p.numero
